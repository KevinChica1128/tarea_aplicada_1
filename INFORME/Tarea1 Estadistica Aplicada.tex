...
~\\ Ubicamos las localizaciones en un mapa, con el software \citeasnoun{R}, mas específicamente con la librería 'ggmap' \cite{G}, para tener una idea geográfica de donde provenían nuestros 500 bloques muestreados, los resultados se aprecian en las siguientes dos gráficas:
\begin{figure}[!htb]
  \centering
...
~\\ Para obtener el mejor modelo posible, aparte del modelo completo (con todas las variables), utilizamos tres métodos de selección de variables, forward, backward y stepwise por medio de \citeasnoun{R} con el paquete 'MASS' \cite{Ma}, los modelos obtenidos por estos métodos, los comparamos con respecto al $R^2_{ajustado}$ y el cuadrado medio del error ($CME=\hat{\sigma}^2$). Los modelos obtenidos fueron los siguientes:
\begin{itemize}

...
x11()
par(mfrow=c(1,2))
hist(cadata$Ingreso_medio,freq = F,col="gray52")
curve(dnorm(x, mean(cadata$Ingreso_medio), sd(cadata$Ingreso_medio)), col = 2, lty = 2, lwd = 2, add=T)
plot(density(cadata$Ingreso_medio),main="Densidad del ingreso medio",col="darkblue")


x11()
par(mfrow=c(1,2))
hist(cadata$Edad_media_de_la_vivienda,freq = F,col="gray52")
